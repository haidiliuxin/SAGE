...
真实 DecisionEvent 后续需补全可用 Arm、全体评分分解、决策前/后状态、奖励分量、切换原因、终止事件及配置版本。当前 `DecisionEvent.as_dict()` 未导出 score_breakdown，最近 50 条检查点事件也不足以重现长任务。完整真实研究日志属于下一步交付，本次提供完整回放轨迹作为样例。
