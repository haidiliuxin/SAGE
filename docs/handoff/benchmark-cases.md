...
### 在界面里手动测同样这些口令时，预算必须给够

评测脚本用的是 **候选预算 10 亿 / 时间 120s**；界面新建任务默认只有
**候选预算 10 万**，这时计划层会按预算裁剪原生攻击（并写入"规划提示"），
"词 + 4 位年份"这类形态会直接不可达。实测量级：

| 任务候选预算 | 计划层的裁剪结果 | 能否解出 `summer2023`（词 + 年份） |
| --- | --- | --- |
| 10 万（界面默认） | S1 规则链缩短为 best66；S7 掩码裁成 `?d?d,!` | ❌ 该口令不在候选空间内 |
| **100 万** | S7 保留 `?d?d?d?d,?d?d,!`（S6 的 `?l?l?l?l` 被牺牲） | ✅ 实测命中（S7，测试 13 万键） |
| **3 亿**（= S1 规则链 2.48 亿 + S6 4616 万 + S7 81.8 万） | 不裁剪，与评测口径一致 | ✅ 实测命中（S1 + S7） |

界面里把"候选预算"改成 `1000000`（快速验证）或 `300000000`（完整口径）即可；
计划页的"规划提示"会明确写出"已缩短为…/需要候选预算 ≥ …"。
