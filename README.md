...
- 真实场景执行（第 1 步）：S1 可切换为 **hashcat 原生词表攻击**（`SAGE_WORDLIST_PATH` 或任务上传的词表文件，由 hashcat 单进程按需流式读取整本字典、不做候选物化），候选管线**惰性流式生成**（`_PlanStream`：只生成被调度到的批次，消费即释放，重启后按已消费批次快进），决策批次大小可配置（`SAGE_DECISION_BATCH_SIZE`），并支持**命中即停**（`stop_on_hit`，命中后立即结束并记 `all_targets_recovered`）；前端可粘贴/上传 `.txt` 词表作为候选或作为原生字典；
