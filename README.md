...
- 自适应切片（第 3 步优化）：原生攻击（词表 × 规则链 / 掩码 / 词表 × 掩码）不再"一次拉完整段键空间"，而是切成**探针 → 提交**的可观测批次——词表类按切片词表文件切、掩码类拆子掩码、进度按**实测条数**推进；调度器每批之后都能按实测产出重新选臂。30 条口径对照：命中 25/30（不降），但命中来源从"S1 占 92%"变为"S1 占 48%"（S7/S5/S3/S2 参与），详见 `docs/handoff/adaptive-slicing.md`；
