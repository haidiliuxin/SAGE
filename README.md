...
- 真实场景执行（第 1 步）：S1 可切换为 **hashcat 原生词表攻击**（`SAGE_WORDLIST_PATH` 指向整本字典，单进程读取、不做候选物化），决策批次大小可配置（`SAGE_DECISION_BATCH_SIZE`），并支持**命中即停**（`stop_on_hit`，命中后立即结束并记 `all_targets_recovered`）；前端可直接粘贴或导入 `.txt` 词表作为高优先级候选；
